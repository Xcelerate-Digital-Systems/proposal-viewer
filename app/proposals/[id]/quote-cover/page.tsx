...
export default function QuoteCoverPage({ params }: { params: { id: string } }) {
  return (
    <AdminLayout>
      {(auth) => (
        <QuoteCoverContent proposalId={params.id} companyId={auth.companyId!} />
      )}
    </AdminLayout>
  );
}

function QuoteCoverContent({ proposalId, companyId }: { proposalId: string; companyId: string }) {
  ...
  return (
    <div className="flex flex-col h-full">
      <QuoteDetailHeader proposalId={proposalId} activeTab="quote-cover" customDomain={customDomain} />
