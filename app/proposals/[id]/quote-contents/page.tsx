...
export default function QuoteContentsPage({ params }: { params: { id: string } }) {
  return (
    <AdminLayout>
      {(auth) => (
        <QuoteContentsContent proposalId={params.id} companyId={auth.companyId!} />
      )}
    </AdminLayout>
  );
}

function QuoteContentsContent({ proposalId, companyId }: { proposalId: string; companyId: string }) {
  ...
  return (
    <div className="flex flex-col h-full">
      <QuoteDetailHeader proposalId={proposalId} activeTab="quote-contents" customDomain={customDomain} />
