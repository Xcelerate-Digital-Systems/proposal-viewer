...
export default function QuotePricingPage({ params }: { params: { id: string } }) {
  return (
    <AdminLayout>
      {(auth) => (
        <QuotePricingContent proposalId={params.id} companyId={auth.companyId!} />
      )}
    </AdminLayout>
  );
}

function QuotePricingContent({ proposalId, companyId }: { proposalId: string; companyId: string }) {
  ...
  return (
    <div className="flex flex-col h-full">
      <QuoteDetailHeader proposalId={proposalId} activeTab="quote-pricing" customDomain={customDomain} />
