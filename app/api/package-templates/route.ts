...
export async function GET(req: NextRequest) {
  try {
    ...
    const limited = await authRateLimit(auth.companyId, 'package-templates');
    ...
    console.error('[api/package-templates] GET:', err);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.error('[api/package-templates] POST:', err);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
