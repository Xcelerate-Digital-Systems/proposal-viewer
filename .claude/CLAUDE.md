...
- **Adding breakdowns** (age / gender / country / placement / etc.): configured per data source via the Apps Script `newSelectMultiple` "Breakdowns" config. Selected values arrive as `configParams.breakdowns`, get passed to Meta via `breakdowns=a,b,c`, and appear in rows under a key matching the breakdown name. `getSchema()` conditionally registers the matching dimension from `Schema.gs > BREAKDOWN_DIMENSIONS` — Looker errors if a row contains a breakdown value for a field the schema didn't declare. Allowlist in `fields.ts > ALLOWED_BREAKDOWNS`. Meta rejects some combinations (e.g. hourly + demographic) with a 400; the error surfaces to Looker Studio verbatim.
