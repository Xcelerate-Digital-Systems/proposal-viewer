...
import { Check, MessageSquare, X, Mail, Phone, AlertTriangle } from 'lucide-react';
...
function ContactLinks({ email, phone, label, muted, hairline }: {
  email?: string;
  phone?: string;
  label: string;
  muted: string;
  ...
  return (
    ...
    >
      <span>{label}</span>
      {email && (
        <a href={`mailto:${email}`} className="inline-flex items-center gap-1.5 underline decoration-dotted underline-offset-4 hover:no-underline" style={{ color: 'inherit' }}>
          <Mail size={13} />
          <span>{email}</span>
        </a>
      )}
      {phone && (
        <a href={`tel:${phone.replace(/\s/g, '')}`} className="inline-flex items-center gap-1.5 underline decoration-dotted underline-offset-4 hover:no-underline" style={{ color: 'inherit' }}>
          <Phone size={13} />
          <span>{phone}</span>
        </a>
      )}
...
export default function ProposalDecisionPanel({
  ...
  const focusRingClass = 'focus-visible:outline-2 focus-visible:outline-offset-1';
  ...
  const submit = async () => {
    ...
    if (activeAction === 'accept' && !agree) return;
    if (activeAction === 'accept' && requireSignature && !signatureData) return;
    if (activeAction === 'revision' && !reason.trim()) return;
  ...
  const switchTab = (tab: 'accept' | 'decline' | 'revision') => {
    setActiveAction(tab);
    setConfirmStep(false);
    setSubmitError(null);
  };
    ...
    >
      ...
      {state === 'accepted' && (
        <>
          ...
          <ContactLinks email={companyEmail} phone={companyPhone} label="Questions?" muted={muted} hairline={hairline} />
        </>
      )}
      ...
      {state === 'declined' && (
        <>
          ...
          <ContactLinks email={companyEmail} phone={companyPhone} label="Changed your mind?" muted={muted} hairline={hairline} />
        </>
      )}
      ...
      {state === 'revision' && (
        <>
          ...
          <ContactLinks email={companyEmail} phone={companyPhone} label="Need to follow up?" muted={muted} hairline={hairline} />
        </>
      )}
          ...
          {isExpired && activeAction === 'accept' ? (
                ...
                >
                  {onRequestRevision && (
                    <button
                      ...
                      className={`px-3 py-2 rounded-lg transition-colors ${focusRingClass}`}
                      style={{ outlineColor: headingColor, color: muted }}
                    >
                      Request Changes
                    </button>
                  )}
            ...
            <form onSubmit={(e) => { e.preventDefault(); submit(); }} noValidate>
                ...
                >
                  {onAccept && !isExpired && (
                    <button
                      ...
                      onClick={() => switchTab('accept')}
                      className={`px-3 py-2 rounded-lg transition-colors ${focusRingClass} ${activeAction === 'accept' ? 'shadow-sm' : ''}`}
                      style={{
                  ...
                  {onRequestRevision && (
                    <button
                      ...
                      onClick={() => switchTab('revision')}
                      className={`px-3 py-2 rounded-lg transition-colors ${focusRingClass} ${activeAction === 'revision' ? 'shadow-sm' : ''}`}
                      style={{
                        outlineColor: headingColor,
                  ...
                  {onDecline && (
                    <button
                      ...
                      onClick={() => switchTab('decline')}
                      className={`px-3 py-2 rounded-lg transition-colors ${focusRingClass} ${activeAction === 'decline' ? 'shadow-sm' : ''}`}
                      style={{
                        outlineColor: headingColor,
                        ...(activeAction === 'decline'
                          ? { backgroundColor: bodyBg, color: headingColor, fontWeight: 600 }
              ...
              {activeAction !== 'accept' && (
                <div className="text-left mb-5">
                  ...
                  <textarea
                    ...
                    className={`w-full px-3 py-2.5 rounded-lg text-base md:text-sm transition-colors ${focusRingClass}`}
                    style={{
                      border: `1px solid ${hairline}`,
                      backgroundColor: bodyBg,
                      color: bodyText,
              ...
              <button
                ...
                className={`w-full px-6 py-3 rounded-lg text-sm transition-colors disabled:opacity-30 disabled:cursor-not-allowed inline-flex items-center justify-center gap-2 tracking-wider uppercase ${focusRingClass}`}
                style={{
                  outlineColor: headingColor,
                  backgroundColor:
                    activeAction === 'accept'
