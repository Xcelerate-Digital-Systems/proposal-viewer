...
export default function MilestonesSection({
  ...
  return (
    <div className={`rounded-lg border p-4 transition-colors ${milestones.enabled ? `${TEAL_BORDER} ${TEAL_BG}` : 'border-edge-strong'}`}>
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-center gap-2">
          <Banknote size={15} className={milestones.enabled ? TEAL_TEXT : 'text-faint'} />
          <span className="text-sm font-medium text-ink">Milestone Payments</span>
        </div>
      ...
      {milestones.enabled && (
        <div className="space-y-3">
          {milestones.payments.map((payment, idx) => {
            ...
            return (
              <div key={payment.id} className="rounded-lg border border-teal/15 bg-white p-3 space-y-2.5">
                {/* Header row */}
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    className={`flex-1 px-2.5 py-1.5 rounded border border-edge-strong bg-white text-ink text-sm focus:outline-none focus:ring-2 ${TEAL_FOCUS} placeholder:text-faint`}
                  />
                ...
                <div className="flex items-center gap-2 ml-7">
                  <select
                    ...
                    className={`w-32 px-2 py-1.5 rounded border border-edge-strong bg-white text-ink text-xs focus:outline-none focus:ring-2 ${TEAL_FOCUS}`}
                  >
                ...
                <div className="ml-7">
                  <input
                    ...
                    className={`w-full px-2.5 py-1.5 rounded border border-edge-strong bg-white text-ink text-xs focus:outline-none focus:ring-2 ${TEAL_FOCUS} placeholder:text-faint`}
                  />
                </div>
              </div>
            );
          ...
          <button
            type="button"
            onClick={onAdd}
            className={`w-full flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg border border-dashed border-teal/30 ${TEAL_TEXT} text-xs font-medium hover:bg-teal/[0.03] transition-colors`}
          >
            <Plus size={14} />
            Add Payment
          </button>
          ...
          {/* Warning */}
          {milestoneWarning && (
            <div className="flex items-start gap-2 px-3 py-2 rounded-lg bg-orange-50 text-xs text-orange-700">
          ...
          {projectTotal > 0 && (
            <div>
              ...
              <div className="flex justify-between mt-1 text-detail text-faint">
