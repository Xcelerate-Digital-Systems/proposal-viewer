...
export default function DynamicFieldMenu({ isOpen, onToggle, onClose, onInsert }: DynamicFieldMenuProps) {
  ...
  return (
    <div className="relative shrink-0" ref={ref}>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 mt-1 w-56 bg-white border border-edge-strong rounded-lg shadow-lg z-50 py-1">
          <div className="px-3 py-1.5 text-2xs font-semibold uppercase text-faint tracking-wider">
          ...
          {DYNAMIC_FIELDS.map((f) => (
            ...
            >
              <div>
                <span className="text-sm text-prose">{f.label}</span>
                <p className="text-2xs text-faint">{f.description}</p>
              </div>
