...
export default function TextPageFormPanel({ form, companyId, onUpdate }: TextPageFormPanelProps) {
  return (
    <div className="flex-1 min-h-0 overflow-y-auto space-y-5">
      ...
      {/* Enabled toggle */}
      <div className="flex items-center justify-between p-4 rounded-2xl border border-edge-strong bg-white">
        <div>
      ...
      {form.enabled && (
        <div className="space-y-5 p-4 rounded-2xl border border-edge-strong bg-white">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex items-center gap-2">
                <span className="text-detail text-faint">Show title in viewer</span>
                <button
                  type="button"
                  role="switch"
                  ...
                  className={`relative inline-flex h-4 w-7 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-teal/20 ${
                    form.show_title ? 'bg-teal' : 'bg-edge-hover'
                  }`}
                >
                  <span
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="rounded-lg border border-edge-strong bg-surface p-3 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-teal/20 ${
                    form.show_member_badge ? 'bg-teal' : 'bg-edge-hover'
                  }`}
                >
                  <span
              ...
              {form.show_member_badge && companyId && (
                <div>
                  <label className="block text-2xs font-medium text-faint mb-1">
                    Team Member
                  </label>
                  <PreparedBySelector
                    companyId={companyId}
            ...
            <div className="rounded-lg border border-edge-strong bg-surface p-3 space-y-2">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-teal/20 ${
                    form.show_client_logo ? 'bg-teal' : 'bg-edge-hover'
                  }`}
                >
                  <span
                    className={`pointer-events-none inline-block h-4 w-4 rounded-full bg-white shadow-sm ring-0 transition-transform duration-200 ${
