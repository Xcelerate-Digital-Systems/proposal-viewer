...
export default function TemplatePackagesTab({ templateId, companyId }: TemplatePackagesTabProps) {
  ...
  const savePkg = useCallback(async (
    ...
    try {
      const res = await fetch(`/api/templates/packages?id=${id}`, {
        method: 'PUT',
  ...
  const addPage = useCallback(async () => {
    ...
    setAdding(false);
  }, [templateId, companyId, selectPage, toast]);
  ...
  const deletePage = useCallback(async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/templates/packages?id=${id}`, { method: 'DELETE' });
      if (res.ok) {
