...
export default function PackagesTab({ proposalId }: PackagesTabProps) {
  ...
  const savePkg = useCallback(async (
    ...
    try {
      const res = await fetch('/api/proposals/packages', {
        ...
        body: JSON.stringify({
          proposal_id: proposalId, enabled: data.enabled, position: currentPosition,
  ...
  const previewPackages: ProposalPackages = {
    id: 'preview', proposal_id: proposalId, company_id: '', enabled: form.enabled,
    position, indent: 0, title: form.title, intro_text: form.intro_text,
    packages: form.packages, footer_text: form.footer_text,
    styling: form.styling, created_at: '', updated_at: '',
  };
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 flex gap-6 p-6 h-full">
      {/* ── Left: Form ─────────────────────────────────────────── */}
      <div className={`flex-1 min-w-0 overflow-y-auto ${showPreview ? 'max-w-[55%]' : ''}`}>
        ...
        {form.enabled ? (
          ...
          <div className="rounded-lg border border-dashed border-gray-200 bg-gray-50 py-12 text-center">
            ...
            <p className="text-xs text-gray-300">Toggle the switch above to add a packages page to this proposal</p>
          </div>
        )}
      </div>
...
/* ------------------------------------------------------------------ */
/*  TierEditor sub-component (reduces main component length)           */
/* ------------------------------------------------------------------ */
