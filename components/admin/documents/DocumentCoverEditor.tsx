...
export default function DocumentCoverEditor({ document: doc, onSave, onCancel }: DocumentCoverEditorProps) {
  const [coverEnabled, setCoverEnabled] = useState(doc.cover_enabled);
  const [subtitle, setSubtitle] = useState(doc.cover_subtitle || '');
  const [buttonText, setButtonText] = useState(doc.cover_button_text || 'START READING');
  const [imagePath, setImagePath] = useState(doc.cover_image_path || '');
  const [imageUrl, setImageUrl] = useState<string | null>(null);
  const [uploading, setUploading] = useState(false);
  const [saving, setSaving] = useState(false);
  const fileRef = useRef<HTMLInputElement>(null);

  // Branding state for preview
  ...
  useEffect(() => {
    if (doc.company_id) {
      fetch(`/api/company/branding?company_id=${doc.company_id}`)
  ...
  const handleImageUpload = async (file: File) => {
    setUploading(true);
    const filePath = `covers/${doc.id}-${Date.now()}.${file.name.split('.').pop()}`;
  ...
  const handleSave = async () => {
    ...
    await supabase.from('documents').update({
      ...
      cover_button_text: buttonText || 'START READING',
    }).eq('id', doc.id);
    setSaving(false);
    onSave();
  };
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. Company Capabilities Overview"
            ...
            />
            <p className="text-xs text-gray-400 mt-1">Displayed below the document title</p>
          </div>
