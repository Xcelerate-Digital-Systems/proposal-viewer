  ...
  return (
    <>
      {item.type === 'webpage' ? (
        ...
        <div className="w-full h-full flex flex-col text-left overflow-hidden bg-[#f5f5f5]">
          {/* Mini status bar */}
          <div className="px-3 pt-2.5 pb-1.5 flex items-center justify-between shrink-0">
            <span className="text-2xs font-semibold text-ink/70">9:41</span>
            <div className="flex items-center gap-0.5">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className={`w-[2px] rounded-sm ${i <= 3 ? 'bg-ink/50' : 'bg-ink/15'}`}
                  style={{ height: 2 + i * 1.5 }} />
              ))}
            </div>
          </div>
          {/* Sender header */}
          <div className="px-3 pb-2 border-b border-edge shrink-0 flex items-center gap-2">
            ...
            <span className="text-2xs font-semibold text-ink truncate">Your Brand</span>
          </div>
          {/* Message bubble */}
          <div className="flex-1 px-3 py-2.5 flex flex-col justify-end overflow-hidden">
            <div className="bg-[#e9e9eb] rounded-2xl rounded-bl-lg px-3 py-2 max-w-[90%]">
              <p className="text-2xs leading-relaxed text-ink line-clamp-5 whitespace-pre-line">
                {item.sms_body || 'No content'}
              </p>
            </div>
            <p className="text-[9px] text-faint mt-0.5 ml-1">Delivered</p>
          </div>
        </div>
      ) : thumbnailUrl ? (
