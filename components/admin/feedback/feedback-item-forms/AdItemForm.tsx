                  ...
                  <div className="px-3 pb-3 pt-2 space-y-2">
                    <div>
                      ...
                      <textarea
                        ...
                        className="w-full px-3 py-2 bg-white border border-edge-strong rounded-lg text-sm text-ink placeholder:text-faint focus:outline-none focus:ring-2 focus:ring-teal/20 resize-y min-h-[96px]"
                      />
                    </div>
                    <div>
                      <label className="flex items-center gap-1 text-2xs font-semibold uppercase tracking-wide text-faint mb-1">
...
function CtaDropdown({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  return (
    <div className="relative" ref={ref}>
      <label className="block text-xs font-medium text-dim uppercase tracking-wider mb-1">CTA</label>
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
