...
export default function BusinessDetailsCard({ companyId, isOwner }: Props) {
  ...
  return (
    <div className="bg-white border border-edge rounded-2xl p-5 space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Field label="Phone" disabled={!isOwner}>
          <input
            ...
            disabled={!isOwner}
            className="w-full px-3 py-2 rounded-lg border border-edge text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/40 disabled:bg-surface placeholder:text-faint"
          />
        </Field>
      ...
      <div className="pt-4 border-t border-edge">
        <h3 className="text-2xs font-semibold uppercase tracking-wide text-muted mb-3">
          Quote number format
        </h3>
        <div className="grid grid-cols-1 sm:grid-cols-[1fr,140px,1fr] gap-4 items-start">
          ...
          <Field label="Pad width" disabled={!isOwner}>
            <input
              ...
              disabled={!isOwner}
              className="w-full px-3 py-2 rounded-lg border border-edge text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/40 disabled:bg-surface placeholder:text-faint tabular-nums"
            />
          </Field>
          <div className="pt-[22px]">
            <div className="text-xs text-faint mb-1">Preview</div>
            <div className="px-3 py-2 rounded-lg bg-surface border border-edge text-sm font-medium tabular-nums">
              {previewExample}
            </div>
          </div>
        </div>
